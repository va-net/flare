<?php
Page::setTitle('Manage Fleet - ' . Page::$pageData->va_name);
...
<div id="content" class="text-black dark:text-white">
    <div class="flex w-full p-5 dark:bg-gray-600 bg-gray-100 py-7 mb-4 items-center">
        ...
        <a href="/admin/fleet/new" class="inline-block px-3 py-1 text-lg font-semibold rounded-md shadow-md hover:shadow-lg bg-primary text-primary-text focus:outline-none focus:ring-2 focus:ring-transparent focus:ring-offset-1 focus:ring-offset-black dark:focus:ring-offset-white">
            Add Aircraft
        </a>
    </div>
    <form method="post" x-ref="deleteaircraft">
        <input type="hidden" name="action" value="deleteaircraft" />
        <input type="hidden" name="delete" value="" x-ref="deleteaircraft-id" />
    </form>
    ...
    <div class="md:px-5 px-2 max-w-full">
        <div class="table-wrapper">
            <table class="table">
                ...
                <tbody>
                    <?php foreach (Page::$pageData->fleet as $aircraft) : ?>
                        <tr class="hover:bg-black/20 cursor-pointer" @click="window.location.href = '/admin/fleet/<?= $aircraft->id ?>'">
                            <td>
